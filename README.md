...
Профиль теперь также может управлять Android split tunneling по приложениям: `tun.exclude-package` исключает выбранные приложения из VPN целиком, `tun.include-package` делает обратный режим, а `tun.exclude-package-file` / `tun.include-package-file` позволяют вынести списки пакетов в отдельные файлы.
