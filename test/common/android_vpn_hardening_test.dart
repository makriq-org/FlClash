...
void main() {
  ...
  test('android compatibility rules preserve bypass domains without localhost proxy', () {
  ...
  test('android profile compatibility injects rules and sniffer for hardened vpn mode', () {
  ...
  test('android profile compatibility still enables sniffer without bypass domains', () {
    final patched = applyAndroidVpnProfileCompatibility(
      {
  ...
  test('android profile compatibility preserves explicit sniffer settings', () {
    ...
    expect((http['ports'] as List).cast<String>(), containsAll(['80', '8080-8880']));
    expect(sniff.keys, containsAll(['HTTP', 'TLS', 'QUIC']));
  });

  test('android profile compatibility stays inert when hardening is inactive', () {
