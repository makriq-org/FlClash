...
### Очистка релизного канала

- Выпускать следующий продовый релиз только из очищенной ветки `main`.
- Удалить из форка старые ветки, унаследованные от апстрима, чтобы история релизов и обновлений оставалась однозначной.
- Оставить самостоятельную линию `makriq` и Android-самообновление на одном поддерживаемом основном бранче.
