...
- Перед попыткой получить список установленных приложений FlClash показывает короткую подсказку на русском и сразу пробует доступ. На Android-оболочках, которые сами показывают системный запрос на такой доступ, эта подсказка появляется прямо перед системным окном. Эта подсказка показывается только один раз вообще, после чего FlClash запоминает сам факт показа. Если пользователь откажется или оболочка не даст доступ, профиль всё равно поднимется, но mask/regex-правила будут временно пропущены, а точные package names продолжат работать.
