...
- Перед первым обращением к списку установленных приложений FlClash кратко объясняет, зачем это нужно. Если пользователь не разрешит доступ, профиль всё равно поднимется, но mask/regex-правила будут временно пропущены, а точные package names продолжат работать.
