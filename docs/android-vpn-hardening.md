...
Дополнительно профиль теперь может задавать split tunneling по приложениям через `tun.exclude-package`, `tun.include-package`, а также через файловые списки `tun.exclude-package-file` и `tun.include-package-file`. Эти поля транслируются в Android `VpnService` и управляют обходом VPN именно на уровне приложений, а не отдельных адресов. Подробный формат с примерами описан в [android-profile-split-tunneling.md](android-profile-split-tunneling.md).
